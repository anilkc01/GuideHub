      ...
      <div className="flex items-center gap-0.5">
        {[...Array(5)].map((_, i) => (
          <Star
            ...
            className={
              i < Math.floor(rating) ? "text-yellow-500" : "text-zinc-700"
            }
          />
        ))}
  ...
  const fetchData = async () => {
    try {
      ...
      if (foundMyOffer) {
        ...
        setFormData({
          amount: foundMyOffer.amount,
          message: foundMyOffer.message,
        });
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-24 pb-12 font-sans">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-md rounded-[2rem] p-8 relative animate-in zoom-in-95 duration-200">
            ...
            <h2 className="text-xl font-serif font-bold mb-6 italic">
              {myOffer ? "Refine Your Offer" : "Place Your Bid"}
            </h2>
            <form onSubmit={handleSubmitOffer} className="space-y-5">
              ...
              <button
                disabled={submitting}
                className="w-full bg-white text-black py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-400 transition-all active:scale-95"
              >
                {submitting
                  ? "Processing..."
                  : myOffer
      ...
      <div className="max-w-7xl mx-auto px-6 mb-6">
        <button
          onClick={() => navigate(-1)}
          className="group flex items-center gap-3 bg-white/5 hover:bg-white/10 px-4 py-2 rounded-xl border border-white/10 transition-all"
        >
          <ArrowLeft
            size={14}
      ...
      <main className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 bg-zinc-900/40 border border-white/10 rounded-[2.5rem] p-8 md:p-10">
          <h1 className="text-3xl md:text-5xl font-serif font-bold mb-6 italic leading-tight">
            {plan.title}
          </h1>
          ...
          <section className="space-y-12">
            <div>
              <h3 className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-black mb-4">
                The Goal
              </h3>
              <p className="text-zinc-300 leading-relaxed text-sm font-medium italic bg-white/[0.01] p-5 rounded-2xl border border-white/5">
                "{plan.description}"
              </p>
            </div>
            ...
            {/* STEPPER ITINERARY */}
            <div>
              <h3 className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-black mb-8">
                Planned Itinerary
              </h3>
              <div className="relative ml-4 space-y-0">
                ...
                {plan?.itinerary?.map((item, idx) => (
                  <div
                    key={idx}
                    className="relative flex items-start gap-6 pb-6 last:pb-0 group"
                  >
                    ...
                    <div className="pt-0.5">
                      <span className="block text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1 group-hover:text-emerald-500 transition-colors">
                        Day 0{idx + 1}
                      </span>
                      <p className="text-sm font-bold text-white/80 group-hover:text-white transition-colors leading-relaxed">
                        {item.activity}
                      </p>
                    </div>
                  </div>
            ...
            <div className="pt-8 border-t border-white/5">
              <p className="text-[9px] font-black uppercase tracking-[0.3em] text-zinc-600 mb-4">
                Planned By
              </p>
              <div
              ...
              >
                <div className="w-10 h-10 shrink-0 rounded-full border border-white/10 overflow-hidden bg-zinc-900 flex items-center justify-center transition-colors group-hover:border-emerald-500/50">
                  {plan.trekker?.dp ? (
                    <img
                      src={`http://localhost:5002/${plan.trekker.dp}`}
                      className="w-full h-full object-cover"
                      alt="trekker"
                    />
                  ) : (
                    <User size={20} className="text-zinc-600" strokeWidth={2} />
                  )}
                </div>
                <div>
                  <p className="text-xs font-bold text-white group-hover:text-emerald-400 transition-colors">
                    {plan.trekker?.fullName}
                  </p>
        ...
        <aside className="lg:col-span-4 flex flex-col gap-6 sticky top-28">
          <div className="bg-zinc-900/40 border border-white/10 rounded-[2.5rem] p-6 flex flex-col min-h-[500px] shadow-2xl">
            <h3 className="text-lg font-serif font-bold mb-6 italic">
              Active Offers
            </h3>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-1 custom-scrollbar">
              {bids.length > 0 ? (
                bids.map((bid) => (
                  <div
                    key={bid.id}
                    className={`p-4 border rounded-2xl space-y-3 transition-all ${bid.bidderId === userData.id ? "bg-emerald-500/5 border-emerald-500/20" : "bg-white/5 border-white/5"}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5 cursor-pointer hover:opacity-80">
                        <div className="w-8 h-8 shrink-0 rounded-full border border-white/10 overflow-hidden bg-zinc-800 flex items-center justify-center">
                          {bid.bidder?.dp ? (
                            <img
                    ...
                    <p className="text-[10px] text-zinc-500 italic leading-relaxed px-2 border-l border-white/10 line-clamp-2">
                      "{bid.message}"
                    </p>
                  </div>
                ))
            ...
            <button
              ...
              className="mt-6 w-full flex items-center justify-center gap-2 bg-white text-black py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-400 transition-all active:scale-95 shadow-lg shadow-white/5"
            >
              {myOffer ? (
                <>
                  <Edit3 size={14} /> Update Offer
                </>
              ) : (
                <>
