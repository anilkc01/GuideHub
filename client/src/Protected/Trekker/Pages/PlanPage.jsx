...
const MyPlanDetails = ({ onLogout }) => {
  ...
  const [isCompleting, setIsCompleting] = useState(false); // New loading state
  ...
  const handleMarkCompleted = async () => {
    if (!plan?.Trip?.id)
      return toast.error("No active trip found for this plan");

    setIsCompleting(true);
    try {
      await api.put(`/trips/mark-completed/${plan.Trip.id}`);
      toast.success("Congratulations on finishing your journey!");
      await fetchData();
    } catch (err) {
      toast.error(
  ...
  return (
    <div className="h-screen bg-black text-white flex flex-col overflow-hidden font-sans">
      ...
      <ChatBox currentUser={userData} autoSelect={activeChatPartner} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 h-[calc(100vh-180px)]">
          <div className="lg:col-span-8 overflow-y-auto pr-8 custom-scrollbar scroll-smooth">
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              ...
              plan.Trip?.status === "upcoming" ? (
                <button
                  disabled={isCompleting}
                  onClick={handleMarkCompleted}
                  className="group flex items-center gap-2 text-[10px] font-black uppercase px-5 py-2 rounded-xl border border-emerald-500 bg-emerald-500 text-black hover:bg-white hover:border-white transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] active:scale-95 disabled:opacity-50"
                >
                  {isCompleting ? (
                  ...
                  <div
                    className={`flex items-center gap-2 text-[10px] font-black uppercase px-4 py-2 rounded-xl border ${
                      plan.status === "open"
                        ...
                        : plan.status === "completed" &&
                            plan.Trip?.status === "completed"
                          ? "text-emerald-400 border-emerald-500/20 bg-emerald-500/20"
                  ...
                  {plan.status === "completed" &&
                    plan.Trip?.status === "completed" && (
                      <button
                        ...
                        className="flex items-center gap-2 text-[10px] font-black uppercase px-4 py-2 rounded-xl bg-white text-black hover:bg-emerald-400 hover:text-black transition-all active:scale-95 animate-in fade-in slide-in-from-left-4 duration-500"
                      >
                        <Edit3 size={14} />
                        Share Your Story
                      </button>
            ...
            <div className="space-y-12">
              <section>
                ...
                <p className="text-zinc-300 text-sm leading-relaxed font-medium bg-white/2 p-5 rounded-3xl border border-white/5">
                  {plan.description}
                </p>
              </section>
              ...
              <section className="pb-20">
                ...
                <div className="relative ml-4 space-y-0">
                  <div className="absolute left-2.75 top-2 bottom-2 w-px bg-linear-to-b from-emerald-500/50 via-white/10 to-transparent" />
                  {plan?.itinerary?.map((item, idx) => (
                    ...
                    >
                      <div className="relative z-10 shrink-0 mt-1">
                        <div className="w-6 h-6 rounded-full bg-black border border-white/20 flex items-center justify-center group-hover:border-emerald-500 transition-colors">
              ...
              {bids.length > 0 ? (
                bids.map((bid) => (
                  <OfferCard
                    ...
                    onMessage={() =>
                      setActiveChatPartner({ partner: bid.bidder })
                    }
                  />
                ))
              ) : (
