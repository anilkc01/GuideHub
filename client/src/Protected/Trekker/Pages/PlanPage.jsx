...
import { ArrowLeft, Calendar, MapPin, DollarSign, Edit3, Loader2, Trash2 } from "lucide-react";
...
const MyPlanDetails = ({ onLogout }) => {
  ...
  return (
    <div className="h-screen bg-black text-white flex flex-col overflow-hidden font-sans">
      ...
      {/* ── Floating Chat Widget (self-contained) ── */}
                ...
                <div className="flex gap-2">
                  {!isDeleting ? (
                    ...
                    <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 p-2 rounded-2xl animate-in fade-in zoom-in-95">
                      <span className="text-[9px] font-black uppercase px-2 text-red-500">Cancel Plan?</span>
                      <button onClick={handleDelete} className="px-3 py-1 bg-red-500 text-white text-[9px] font-black uppercase rounded-lg">Yes</button>
                      <button onClick={() => setIsDeleting(false)} className="px-3 py-1 bg-white/10 text-white text-[9px] font-black uppercase rounded-lg">No</button>
                    </div>
                  )}
                </div>
              )}
              ...
              {bids.length > 0 ? (
                bids.map((bid) => (
                  <OfferCard
                    ...
                    onMessage={() => setActiveChatPartner({ partner: bid.bidder })}
                  />
                ))
              ) : (
